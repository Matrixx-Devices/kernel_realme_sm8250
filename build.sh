...
clone_k() {
    cd $KERNEL_PATH
    git clone https://github.com/AAMIRR-ALI/AnyKernel3 --depth=1 $KERNEL_PATH/AnyKernel3
}
...
make_anykernel3_zip() {
    ...
    if [ "$KernelSU" == "true" ]; 
    then
    	zip_name="RDX-KSU-v4.19.$(grep "^SUBLEVEL =" Makefile | awk '{print $3}')-$(date +"%Y%m%d-%H%M").zip"
    else
        zip_name="RDX-v4.19.$(grep "^SUBLEVEL =" Makefile | awk '{print $3}')-$(date +"%Y%m%d-%H%M").zip"
    fi
...
if [ -d $Anykernel ]; then
    ...
    echo -e "${red}Anykernel is missing, Cloning...${clear}"
